...
from .data import PcaAnalysis, YieldData, convert_ust_tenor_to_years
...
links_intro = [
    html.A(
        ...
        href="https://research-doc.credit-suisse.com/docView?language=ENG&source=emfromsendlink&format=PDF&document_id=1001969281&extdocid=1001969281_1_eng_pdf&serialid=Coz8ZUCgL92gmMydSBULHIsgm%2b9q2TPfBu%2bX1XhViIs%3d",
    ),
    html.Br(),
    html.A(
        "NY Federal Reserve: Deconstructing the Yield Curve",
...
def init_dashboard_pca(server):
    ...
    df_ust_plot.index = [
        convert_ust_tenor_to_years(ust_tenor) for ust_tenor in df_ust_plot.index
    ]

    df_eigen_vectors = yield_data.pca.eigen_vectors
    df_eigen_vectors = df_eigen_vectors.divide(df_eigen_vectors["30yr"], axis=0).round(
        2
    )

    df_eigen_table = df_eigen_vectors[:]
    df_eigen_table.index += 1
    df_eigen_table = df_eigen_table.reset_index().rename(
        columns=dict(index="Eigen Vector")
    )
    df_eigen_table["Expl. Var."] = yield_data.pca.explained_variance.explained_variance[
        : len(df_eigen_table)
    ].round(0)
    ...
    graph_yield_curve = dcc.Graph(
        id="graph-ust-curve",
        figure=dict(
            data=[go.Scatter(x=df_ust_plot.index, y=df_ust_plot[last_date] / 100)],
            layout=dict(
    ...
    graph_ust_normality = dcc.Graph(
        ...
        figure=dict(
            data=[
                get_heatmap_plot(
                    yield_data.gaussian_test_by_year,
                    colorscale=[
                        [0, "rgb(255,255,255)"],
                        [0.10, "rgb(90,220,200)"],
                        [1, "rgb(43, 204, 162)"],
    ...
    graph_eigen_scatter = dcc.Graph(
        id="graph-eigen-scatter", figure=get_eigen_scatter_plot(yield_data)
    )
...
def get_eigen_scatter_plot(yield_data, x="2yr", y="7yr", z="30yr"):
    ...
    hover_template = (
        "i: %{x:.0f}<br>i: %{y}<br>i: %{z}<extra></extra>".replace("i", x, 1)
        .replace("i", y, 1)
        .replace("i", z, 1)
    )
    ...
    fig = go.Figure(
        data=[
            go.Scatter3d(
                ...
                z=data[z],
                mode="markers",
                marker=dict(color=color_palette[0], size=5, opacity=0.5),
        ...
        + [
            go.Scatter3d(
                ...
                z=dataframe[z],
                mode="lines",
                line=dict(dash="dash", color=color_palette[eigen + 1]),
...
def get_eigen_line_plot(df_eigen_table, yield_data):
    fig = go.Figure(
        data=[
            go.Scatter(
                x=df_eigen_table.loc[i].index,
                y=df_eigen_table.loc[i].values,
                mode="lines",
        ...
        + [
            go.Scatter(
                x=df_eigen_table.loc[0].index,
                y=[0 for i in df_eigen_table.loc[0].index],
                mode="lines",
