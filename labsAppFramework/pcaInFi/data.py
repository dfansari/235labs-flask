...
def test_normality_by_year(df_yield_chg, bucketed_years=1):
    ...
    df_gaussian_test = pd.pivot_table(
        pd.DataFrame(dict(year=years, tenor=tenors, p_value=p_values)),
        index="tenor",
        columns="year",
        values="p_value",
        aggfunc="mean",
...
class PcaAnalysis:
    def __init__(self, data, normalized=False, tenors=None):
        ...
        pca.fit(X)
        eigen_vectors = pd.DataFrame(pca.components_).rename(
            columns={i: col for i, col in enumerate(self.data.columns)}
        )
        ...
        explained_variance = (
            ...
            .rename(columns={"index": "eigen_vector", 0: "explained_variance"})
            .set_index("eigen_vector")
        )
    ...
    def get_eigen_plot_data(self, x, y, z, eigen_value=0):
        ...
        eigen_y = [
            (X / eigen.loc[eigen_value, x]) * eigen.loc[eigen_value, y] for X in eigen_x
        ]
        eigen_z = [
            (X / eigen.loc[eigen_value, x]) * eigen.loc[eigen_value, z] for X in eigen_x
        ]
...
class YieldData:
    def __init__(self):
        ...
        self.gaussian_test_by_year = test_normality_by_year(
            self.data_chg, bucketed_years=1
        )
        self.gaussian_test_aggregate = test_normality_by_year(
            self.data_chg, bucketed_years=len(self.data_chg.index.year.unique())
