...
def init_dashboard_citibike(server):
    ...
    stations_ids = [
        col
        for col in df_station_flows.columns
        if col not in ["estimator", "metric", "hour"] and "Unnamed" not in col
    ]

    df_station_metadata = pd.read_csv("data/citibike/2023_station_summary.csv")
    df_station_id_name_map = df_station_metadata[["station_id", "station_name"]].loc[
        df_station_metadata.station_id.isin(stations_ids)
    ]
    df_station_id_name_map = (
        df_station_id_name_map.groupby(by="station_name")[["station_id"]]
        .first()
        .reset_index()
    )
    station_name_id_map = {
        value: name for [name, value] in df_station_id_name_map.values
    }

    init_station_id = "4419.03"
    station_selector = dcc.Dropdown(
        id="station-selector",
        options=station_name_id_map,
        value=init_station_id,
        multi=False,
        clearable=False,
    )
    ...
    dash_app.layout = html.Div(
        children=[
            ...
            html.Div(
                [get_station_metadata_and_status(station_id=init_station_id)],
                id="station-status-div",
            ),
            html.H3("Station Historic Flows"),
            html.P(text_station_history),
            html.Div(
                [graph_station_hourly_trends(station_id=init_station_id)],
                id="station-hourly-div",
            ),
            html.H3("Predicted Station Flows"),
            html.P(text_station_prediction),
            html.Div(
                [get_model_prediction_graph(station_id=init_station_id)],
                id="station-model-div",
            ),
        ],
        id="dash-container",
    )
...
def get_map_chart():
    ...
    df_nyc_population = pd.read_csv(
        "data/citibike/New_York_City_Population_By_Neighborhood_Tabulation_Areas.csv"
    )
    df_station_total = pd.read_csv("data/citibike/2023_station_summary.csv")
    df_station_total["label"] = df_station_total.apply(
        lambda row: f"{row['station_name']}<br>Neighborhood {row['neighborhood']}<br>Station ID {row['station_id']}",
        axis=1,
    )
    ...
    fig.update_layout(
        ...
        mapbox_zoom=10,
        mapbox_center={
            "lat": df_station_total.lat.mean(),
            "lon": df_station_total.lon.mean(),
        },
    )
    fig.update_layout(margin={"r": 10, "t": 10, "l": 10, "b": 10})

    graph_map = dcc.Graph(id="graph-citibike-map", figure=fig)
    return graph_map


def get_total_flows_heatmap_by_neighborhood():
    df_total_flows = pd.read_csv(
        "data/citibike/2023_neighborhood_hourly_total_flows.csv"
    ).set_index("neighborhood")
    df_total_flows = df_total_flows.divide(
        df_total_flows.abs().max(axis=1).max(), axis=0
    )
    ...
    graph_heatmap = dcc.Graph(id="graph-heamap-neighborhood-hourly-total", figure=fig)
    return graph_heatmap


def get_net_flows_heatmap_by_neighborhood():
    df_net_flows = pd.read_csv(
        "data/citibike/2023_neighborhood_hourly_net_flows.csv"
    ).set_index("neighborhood")
    ...
    graph_heatmap = dcc.Graph(id="graph-heamap-neighborhood-hourly-net", figure=fig)
    return graph_heatmap


def get_monthly_total_flows_by_neighborhood():
    df_total_flows = pd.read_csv(
        "data/citibike/2023_neighborhood_monthly_total_flows.csv"
    ).set_index("neighborhood")
    ...
    graph_monthly_neighborhood = dcc.Graph(
        id="graph-neighborhood-monthly-total", figure=fig
    )
...
def graph_station_hourly_trends(station_id):
    # get station data
    df_station_hourly_trends = pd.read_csv(
        "data/citibike/2023_station_hourly_trends.csv"
    )
    df_station_metadata = pd.read_csv("data/citibike/2023_station_summary.csv")
    df_station_metadata["station_id"] = df_station_metadata["station_id"].astype(str)

    # subset station data
    station_metadata = df_station_metadata.loc[
        df_station_metadata.station_id == station_id
    ].iloc[0]
    df_station_hourly = df_station_hourly_trends[
        ["estimator", "metric", "hour", station_id]
    ]
    ...
    for is_positive in [True, False]:
        fig.add_trace(
            go.Bar(
                x=df_station_hourly.loc[
                    (df_station_hourly.net_rides["mean"] >= 0) == is_positive
                ].index,
                y=df_station_hourly.loc[
                    (df_station_hourly.net_rides["mean"] >= 0) == is_positive
                ]["net_rides"]["mean"],
                ...
                marker=dict(
                    color="rgb(81, 174, 245)" if is_positive else "rgb(232, 63, 91)"
                ),
            ),
            row=1,
            col=2,
    ...
    graph_station_hourly = dcc.Graph(id="graph-station-hourly", figure=fig)
...
def get_station_metadata_and_status(station_id):
    ...
    for col in ["lat", "lon", "capacity"]:
        df_station_metadata[col] = pd.to_numeric(
            df_station_metadata[col], errors="coerce"
        )
    ...
    df_station_info = df_station_metadata.loc[
        df_station_metadata.short_name == station_id
    ][
        [
            "name",
            "short_name",
...
def get_model_prediction_graph(station_id):
    ...
    df_r_2s = pd.DataFrame(
        np.array([[k, v["r_2"]] for k, v in station_models_dict.items()])
    )
    ...
    for hour in df_station_hourly.index:
        mean, std = (
            df_station_hourly["outbound_rides"]["mean"][hour],
            df_station_hourly["outbound_rides"]["std"][hour],
        )
    ...
    graph_station_model = dcc.Graph(id="graph-station-model", figure=fig)
